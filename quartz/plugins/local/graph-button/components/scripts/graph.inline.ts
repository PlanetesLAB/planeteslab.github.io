...
(function () {
  ...
  Promise.all([
    ...
    .catch(function (err) {
      ...
      var containers = document.querySelectorAll(".graph-container, .global-graph-container");
  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      var enableDrag = config.drag !== false;
      var enableZoom = config.zoom !== false;
      ...
      var showTags = config.showTags !== false;
      var focusOnHover = config.focusOnHover ?? true;
      var enableRadial = config.enableRadial ?? true;
      ...
      var width = graph.offsetWidth || window.innerWidth * 0.8;
      var height = Math.max(graph.offsetHeight, 300);
